...
export default function FormSelectFile(){
 ...
 const onClickButton = () => {
    ...
    if(inputFile.current.files.length > maxNumFiles){
        alert(`เลือกได้ไม่เกิน ${maxNumFiles}ไฟล์`) // alt + 9+6  = `
        return
        }
...
<form>
    ...
    <div className="form-group mb-3">
        <label htmlFor="MaxFileSize" className="form-select-sm">ขนาดไฟล์สูงสุด(KB)</label>
    ...
    <div className="form-group mb-3">
        <label htmlFor="file" className="file">เลือกไฟล์</label>
